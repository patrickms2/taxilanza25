...
use App\Models\Appointmentsusersusers;

use App\Repositories\AppointmentsusersusersRepositoryInterface;
use Illuminate\Support\Collection;
use Illuminate\Support\Facades\DB;

class AppointmentsusersusersRepository extends BaseRepository implements AppointmentsusersusersRepositoryInterface
{
    public function __construct(Appointmentsusersusers $model)
    {
    ...
    public function findAll($params) : array
    {
        ...
        if (isset($params['filter'])) {
            ...
            if (isset($filter['appointments_usersid'])) {
                $query->where('appointments_usersid', 'like', '%'.$filter['appointments_usersid'].'%');
            }
    ...
    public function create(array $attributes, $currentUser)
    {
        try {
            ...
            $appointmentsusersusers = Appointmentsusersusers::create([
                    'appointments_usersid' => $attributes['appointments_usersid'] ?? null
,
...
,
    ...
    public function update($id, array $attributes, $currentUser)
    {
        try {
            $attributes = $attributes['data'];
            DB::beginTransaction();
            $appointmentsusersusers = Appointmentsusersusers::find($id);
            $appointmentsusersusers->update([
                    'appointments_usersid' => $attributes['appointments_usersid'] ?? null
,
                    'userid' => $attributes['userid'] ?? null
,
