        ...
        {!loading &&
          locations.map((item) => (
            ...
            >
              ...
              >
                <Link
                  href={`/locations/locations-view/?id=${item.id}`}
                  className={
                    'flex-1 px-4 py-6 h-24 flex divide-x-2  divide-stone-300   items-center overflow-hidden`}> dark:divide-dark-700 overflow-x-auto'
                  }
                >
                  <div className={'flex-1 px-3'}>
                    <p className={'text-xs   text-gray-500 '}>taxiid</p>
                    <p className={'line-clamp-2'}>
                      {dataFormatter.taxisOneListFormatter(item.taxiid)}
                    </p>
                  </div>
                  ...
                  <div className={'flex-1 px-3'}>
                    <p className={'text-xs   text-gray-500 '}>last_update</p>
                    <p className={'line-clamp-2'}>
                ...
                <ListActionsPopover
                  onDelete={onDelete}
                  itemId={item.id}
                  pathEdit={`/locations/locations-edit/?id=${item.id}`}
                  pathView={`/locations/locations-view/?id=${item.id}`}
